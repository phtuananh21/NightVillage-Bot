...
Func Example()
	...
	Local $idBtn_Browse = GUICtrlCreateButton("...", 210, 5, 35, 20)
	...
	Local $idBtn_Calculate = GUICtrlCreateButton("Calculate", 400, 40, 65, 25)
	Local $idEdt_Hash = GUICtrlCreateEdit("Hash Digest", 5, 45, 350, 40)
	...
	While 1
		Switch GUIGetMsg()
			...
			Case $idBtn_Browse
				...
				GUICtrlSetData($idEdt_Hash, "Hash Digest") ; Reset the hash digest label.
			...
			Case $idBtn_Calculate
				...
				If StringStripWS($sRead, $STR_STRIPALL) <> "" And FileExists($sRead) Then ; Check there is a file available to find the hash digest
					...
					GUICtrlSetData($idEdt_Hash, $dHash) ; Set the hash digest label with the hash data.
				EndIf
		EndSwitch
	WEnd
