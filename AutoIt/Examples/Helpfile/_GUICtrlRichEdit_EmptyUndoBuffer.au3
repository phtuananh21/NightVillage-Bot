...
Func Example()
	...
	$g_idMni_Empty = GUICtrlCreateMenuItem("Empty Undo buffer", $g_idMnu)
...
Func SetMenuTexts($hWnd, $hMenu)
	If _GUICtrlRichEdit_CanUndo($hWnd) Then
		...
		_GUICtrlMenu_SetItemEnabled($hMenu, $g_idMni_Empty, True, False)
	Else
		_GUICtrlMenu_SetItemText($hMenu, $g_idMni_Undo, "Undo", False)
		_GUICtrlMenu_SetItemEnabled($hMenu, $g_idMni_Undo, False, False)
		_GUICtrlMenu_SetItemEnabled($hMenu, $g_idMni_Empty, False, False)
	EndIf
