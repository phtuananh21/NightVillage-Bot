...
Func Example()
	...
	Local $idBtn_Encrypt = GUICtrlCreateButton("Encrypt", 355, 70, 65, 25)
		...
		Switch GUIGetMsg()
			...
			Case $idBtn_SourceBrowse
				...
				If @error Then
					ContinueLoop
				EndIf
				GUICtrlSetData($idInp_Source, $sFilePath) ; Set the inputbox with the filepath.
			...
			Case $idBtn_DestinationBrowse
				...
				If @error Then
					ContinueLoop
				EndIf
				GUICtrlSetData($idInp_Destination, $sFilePath) ; Set the inputbox with the filepath.
			...
			Case $idBtn_Encrypt
				$sSourceRead = GUICtrlRead($idInp_Source) ; Read the source filepath input.
				$sDestinationRead = GUICtrlRead($idInp_Destination) ; Read the destination filepath input.
				$sPasswordRead = GUICtrlRead($idInp_Password) ; Read the password input.
