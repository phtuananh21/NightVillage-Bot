...
Func Example()
	; Create GUI
	Local $hGUI = GUICreate("ImageList Get Icon (v" & @AutoItVersion & ")", 400, 300)
	...
	; Show icon handles
	_MemoWrite("Part 1 icon handle .: 0x" & Hex($ahIcons[0]))
	_MemoWrite("Part 2 icon handle .: 0x" & Hex($ahIcons[1]))
	_MemoWrite("")
