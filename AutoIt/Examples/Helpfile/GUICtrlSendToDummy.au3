...
Func Example()
	...
	GUICtrlSendToDummy($g_idDmy_User, 1) ; Set state to be checked Onclick
...
Func OnClick()
	Return GUICtrlSendToDummy($g_idDmy_User) ; Send a message to the dummy control that the close button was selected, which will then proceed to call the function 'OnExit'.
EndFunc   ;==>OnClick
