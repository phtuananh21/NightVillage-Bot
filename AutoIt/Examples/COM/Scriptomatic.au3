...
Global $g_idLbl_WMI_Namespace = GUICtrlCreateLabel("WMI Namespace", 20, 30, 150, 20)
Global $g_idCbo_WMI_Namespace = GUICtrlCreateCombo("WMI_Namespaces", 20, 50, 280, 21)
GUICtrlCreateLabel("WMI Class", 320, 30, 140, 20)
Global $g_idCbo_WMI_Classes = GUICtrlCreateCombo("WMI_Classes", 320, 50, 340, 21)
Global $g_idBtn_AST_Web = GUICtrlCreateButton("Lookup on WWW", 560, 27, 100, 20)
GUICtrlCreateGroup("", 10, 80, 660, 50)
Global $g_idBtn_AST_Run = GUICtrlCreateButton("Run", 20, 100, 50, 20)
Global $idBtn_AST_CIMv2 = GUICtrlCreateButton("CIMv2", 80, 100, 50, 20)
Global $idBtn_AST_WMISource = GUICtrlCreateButton("WMISource", 140, 100, 70, 20)
Global $idBtn_AST_Open = GUICtrlCreateButton("Open", 220, 100, 60, 20)
Global $g_idBtn_AST_Save = GUICtrlCreateButton("Save", 290, 100, 60, 20)
Local $idBtn_AST_Quit = GUICtrlCreateButton("Quit", 360, 100, 60, 20)
GUICtrlCreateGroup("Output", 430, 80, 240, 50)
Global $g_idRdo_AST_RadioDialog = GUICtrlCreateRadio("Dialog", 440, 100, 50, 20)
Global $g_idRdo_AST_RadioText = GUICtrlCreateRadio("Text", 510, 100, 50, 20)
Global $g_idRdo_AST_RadioHTML = GUICtrlCreateRadio("HTML", 570, 100, 50, 20)
Global $g_idEdt_AST_ScriptCode = GUICtrlCreateEdit("One moment...", 20, 140, 640, 390)

GUISetState()

; Initial GUI Settings
GUICtrlSetState($g_idBtn_AST_Web, $GUI_DISABLE)
GUICtrlSetState($g_idBtn_AST_Run, $GUI_DISABLE)
GUICtrlSetState($g_idBtn_AST_Save, $GUI_DISABLE)
GUICtrlSetState($g_idRdo_AST_RadioDialog, $GUI_CHECKED)
...
While 1
	...
	Select
		Case $iMsg = $GUI_EVENT_CLOSE
			ExitLoop
		Case $iMsg = $idBtn_AST_Quit
			ExitLoop
		Case $iMsg = $g_idCbo_WMI_Namespace
			HandleNamespaceChange()
		Case $iMsg = $g_idCbo_WMI_Classes
			ComposeCode()
		Case $iMsg = $g_idBtn_AST_Web
			LookupWeb()
		Case $iMsg = $g_idBtn_AST_Run
			RunScript()
		Case $iMsg = $g_idBtn_AST_Save
			SaveScript()
		Case $iMsg = $idBtn_AST_Open
			OpenScript()
		Case $iMsg = $idBtn_AST_CIMv2
			SetNamespaceToCIMV2()
		Case $iMsg = $idBtn_AST_WMISource
			SetWMIRepository()
		Case $iMsg = $g_idRdo_AST_RadioDialog Or _
				$iMsg = $g_idRdo_AST_RadioText Or _
				$iMsg = $g_idRdo_AST_RadioHTML
			HandleOutputChange()
	EndSelect
WEnd
...
Func LoadWMINamespaces()
	...
	GUICtrlSetData($g_idCbo_WMI_Namespace, $sWaitNamespaces, $sWaitNamespaces)
	...
	GUICtrlSetData($g_idCbo_WMI_Namespace, $sNamespacesCombo, "ROOT\CIMV2")
...
Func HandleNamespaceChange()
	...
	;'''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
	Local $sSelectedNamespace = GUICtrlRead($g_idCbo_WMI_Namespace)

	; Disable the namespace combobox until class load has been completed
	GUICtrlSetState($g_idCbo_WMI_Namespace, $GUI_DISABLE)

	Local $sWMIWaitMsg = "Please wait, trying to load WMI Classes in namespace " & $sSelectedNamespace
	GUICtrlSetData($g_idCbo_WMI_Classes, $sWMIWaitMsg, $sWMIWaitMsg)
	GUICtrlSetData($g_idEdt_AST_ScriptCode, "One moment...", "")
	...
	;'''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
	GUICtrlSetData($g_idEdt_AST_ScriptCode, "", "")

	GUICtrlSetState($g_idCbo_WMI_Namespace, $GUI_ENABLE)
	GUICtrlSetState($g_idBtn_AST_Run, $GUI_DISABLE)
	GUICtrlSetState($g_idBtn_AST_Save, $GUI_DISABLE)
...
Func LoadWMIClasses()
	...
	If Not @error Then
		...
		If $oClassDictionary.Count Then
			...
			GUICtrlSetData($g_idCbo_WMI_Classes, $sClassesCombo, "Select a WMI class")
	...
	If @error Or $oClassDictionary.Count = 0 Then
		...
		;''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
		GUICtrlSetData($g_idCbo_WMI_Classes, "|No dynamic classes found in current namespace.|Select a different namespace", "")
...
Func ComposeCode()
	...
	Local $sSelectedClass = GUICtrlRead($g_idCbo_WMI_Classes)
	...
	If $sSelectedClass <> "Select a WMI class" Then
		...
		GUICtrlSetData($g_idEdt_AST_ScriptCode, $sScriptCode)
		...
		GUICtrlSetState($g_idBtn_AST_Run, $GUI_ENABLE)
		GUICtrlSetState($g_idBtn_AST_Save, $GUI_ENABLE)

		; Enable Web lookup button
		GUICtrlSetState($g_idBtn_AST_Web, $GUI_ENABLE)
	Else
...
Func RunScript()
	...
	FileWrite($sTmpName, GUICtrlRead($g_idEdt_AST_ScriptCode))
...
; ********************************************************************
Func SaveScript()
	Local $sTmpName = FileSaveDialog("Save Script", @DesktopDir, "AutoIt3 Scripts (*.au3)", 16, GUICtrlRead($g_idCbo_WMI_Classes))
...
Func OpenScript()
	...
	If Not @error And $sTmpName <> "" Then
		If FileExists($sTmpName) Then
			GUICtrlSetData($g_idEdt_AST_ScriptCode, FileRead($sTmpName, FileGetSize($sTmpName)))
		EndIf
	EndIf
EndFunc   ;==>OpenScript
...
Func SetNamespaceToCIMV2()
	If StringUpper(GUICtrlRead($g_idCbo_WMI_Namespace)) <> "ROOT\CIMV2" Then
		GUICtrlSetData($g_idCbo_WMI_Namespace, "ROOT\CIMV2", "ROOT\CIMV2")
		HandleNamespaceChange()
	EndIf
...
Func HandleOutputChange()
	...
	If GUICtrlRead($g_idRdo_AST_RadioDialog) = $GUI_CHECKED Then $sChosenFormat = "Dialog"
	If GUICtrlRead($g_idRdo_AST_RadioText) = $GUI_CHECKED Then $sChosenFormat = "Text"
	If GUICtrlRead($g_idRdo_AST_RadioHTML) = $GUI_CHECKED Then $sChosenFormat = "HTML"
