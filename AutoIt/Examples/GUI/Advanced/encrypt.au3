...
Func _Main()
	...
	; Creates main edit
	Local $idEdit_Text = GUICtrlCreateEdit('', 5, 5, 380, 350)

	; Creates the password box with blured/centered input
	Local $idInp_utPass = GUICtrlCreateInput('', 5, 360, 100, 20, BitOR($ES_CENTER, $ES_PASSWORD))
	...
	Local $idBtn_Encrypt = GUICtrlCreateButton('Encrypt', 210, 360, 85, 35)
	Local $idBtn_Decrypt = GUICtrlCreateButton('Decrypt', 300, 360, 85, 35)
	...
	While 1
		Switch GUIGetMsg()
			...
			Case $idBtn_Encrypt
				...
				$dEncrypted = _Crypt_EncryptData(GUICtrlRead($idEdit_Text), GUICtrlRead($idInp_utPass), $iAlgorithm) ; Encrypt the text with the new cryptographic key.
				GUICtrlSetData($idEdit_Text, $dEncrypted)

			Case $idBtn_Decrypt
				; When you press Decrypt

				; Calls the encryption. Sets the data of editbox with the encrypted string
				$dEncrypted = _Crypt_DecryptData(GUICtrlRead($idEdit_Text), GUICtrlRead($idInp_utPass), $iAlgorithm) ; Decrypt the data using the generic password string. The return value is a binary string.
				GUICtrlSetData($idEdit_Text, BinaryToString($dEncrypted))
