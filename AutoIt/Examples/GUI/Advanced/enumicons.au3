...
; Setting variables
Global $g_aidIcon_s[30], $g_aidLabel_s[30]
...
Func _Main()
	...
	Local $hGui = GUICreate("Icon Selector by Ordinal value - v" & $g_sFileVersion, 395, 435, @DesktopWidth / 2 - 192, _
	...
	Local $idInp_File = GUICtrlCreateInput($g_sFilename, 12, 15, 325, 16, -1, $WS_EX_STATICEDGE)
	GUICtrlSetState(-1, $GUI_DROPACCEPTED)
	GUICtrlSetTip(-1, "You can drop files from shell here...")
	Local $idBtn_FileSel = GUICtrlCreateButton("...", 345, 14, 26, 18)
	$g_idBtn_Prev = GUICtrlCreateButton("Previous", 10, 45, 60, 24, $BS_FLAT)
	GUICtrlSetState(-1, $GUI_DISABLE)
	Local $idBtn_Next = GUICtrlCreateButton("Next", 75, 45, 60, 24, $BS_FLAT)
	Local $idBtn_Toggle = GUICtrlCreateButton("by Name", 300, 45, 60, 24, $BS_FLAT)
	...
	For $iCntRow = 0 To 4
		For $iCntCol = 0 To 5
			...
			$g_aidIcon_s[$iCurIndex] = GUICtrlCreateIcon($g_sFilename, $g_iOrdinal * ($iCurIndex + 1), _
					60 * $iCntCol + 25, 70 * $iCntRow + 80)
			$g_aidLabel_s[$iCurIndex] = GUICtrlCreateLabel($g_iOrdinal * ($iCurIndex + 1), _
	...
	While 1
		...
		$sCurFilename = GUICtrlRead($idInp_File)
		...
		Select
			Case $iMsg = $idBtn_FileSel
				$sTmpFile = FileOpenDialog("Select file:", "::{20D04FE0-3AEA-1069-A2D8-08002B30309D}", "Executables & dll's (*.exe;*.dll;*.ocx;*.icl)")
				If @error Then ContinueLoop
				GUICtrlSetData($idInp_File, $sTmpFile) ; GUI will be updated at next iteration
			Case $iMsg = $g_idBtn_Prev
				$g_iDir = -1
			...
			Case $iMsg = $idBtn_Next
				$g_iDir = 1
				_GUIUpdate($g_iDir)
			Case $iMsg = $idBtn_Toggle
				If $g_iOrdinal = -1 Then
					$g_iOrdinal = 1
					GUICtrlSetData($idBtn_Toggle, "by Ordinal")
					WinSetTitle($hGui, "", "Icon Selector by Name value - v" & $g_sFileVersion)
				Else
					$g_iOrdinal = -1
					GUICtrlSetData($idBtn_Toggle, "by Name")
					WinSetTitle($hGui, "", "Icon Selector by Ordinal value - v" & $g_sFileVersion)
...
Func _GUIUpdate($iDir)
	ToolTip("")
	If $g_iOrdinal = 1 Then GUICtrlSetState($g_idBtn_Prev, $GUI_DISABLE)
	...
	While 1
		...
		For $iCntRow = 0 To 4
			For $iCntCol = 0 To 5
				...
				Else
					While 1
						$iSet = GUICtrlSetImage($g_aidIcon_s[$iCurIndex], $g_sFilename, $g_iOrdinal * ($iIndex))
				...
				If $iSet = 0 Then
					$iNoIcon += 1
					GUICtrlSetData($g_aidLabel_s[$iCurIndex], "")
				EndIf
				If $g_iOrdinal = -1 Then
					If $iSet Then GUICtrlSetData($g_aidLabel_s[$iCurIndex], -($iIndex))
				Else
					If ($iIndex >= 2 ^ 16) Then
						$iSet = GUICtrlSetImage($g_aidIcon_s[$iCurIndex], $g_sFilename, $g_iOrdinal * (2 ^ 16))
						GUICtrlSetData($g_aidLabel_s[$iCurIndex], "") ; avoid display label
					Else
						GUICtrlSetData($g_aidLabel_s[$iCurIndex], '"' & ($iIndex) & '"')
					EndIf
				EndIf
			Next
		Next
	...
	Else
		If $g_iOrdinal = -1 Then GUICtrlSetState($g_idBtn_Prev, $GUI_ENABLE)
	EndIf
