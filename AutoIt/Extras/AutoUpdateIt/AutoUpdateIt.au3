...
;
; History:
;  - 1.51 - Get COLOR_HOTLIGHT from theme (by argumentum)
...
Local $idGrp_Instal_Details = GUICtrlCreateGroup('Current Installation Details', 5, 5, 340, 70)
...
GUICtrlSetFont(-1, 6)
Local $idGrp_Release = GUICtrlCreateGroup('Latest Public Release', 5, 85, 165, 60)
...
Local $idGrp_Beta = GUICtrlCreateGroup('Latest Beta', 180, 85, 165, 60)
...
Local $idGrp_PreBeta = GUICtrlCreateGroup('Latest Pre-Beta', 180 + 175, 85, 165, 60)
...
; ========================================
Local $idBtn_ReleaseDl = GUICtrlCreateButton('Download Public &Release', 5, 195, 165, 30)
...
Global $g_idProgress_Ctrl = GUICtrlCreateProgress(5, 225, 340, 20)
...
; $idLbl_About_ContactAuthor = GUICtrlCreateLabel('Contact Rob Saunders', 5, 100, 145, 15)
; GUICtrlSetFont(-1, 9, 400, 4)
; GUICtrlSetColor(-1, $g_iHotlightColor)
; GUICtrlSetCursor(-1, 0)
		...
		Else
			...
			If $g_sPreBetaVer <> '' Then
				If _CompareVersions(StringTrimRight($g_sPreBetaVer, 1), $g_sLatestBetaVer) > 0 Then
					...
					GUICtrlSetPos($g_idProgress_Ctrl, 5, 225, 340 + 175, 20)
			...
			If _CompareVersions($g_sReleaseVer, $s_CurrVer) Then
				GUICtrlSetData($idGrp_Release, 'Latest Public Release *New*')
			...
			If _CompareVersions($g_sLatestBetaVer, $s_CurrBetaVer) Then
				GUICtrlSetData($idGrp_Beta, 'Latest Beta *New*')
			...
			If _CompareVersions($g_sPreBetaVer, $s_CurrVer) Then
				GUICtrlSetData($idGrp_PreBeta, 'Latest pre-Beta *New*')
				GUICtrlSetColor($idGrp_PreBeta, $g_iHotlightColor)
			EndIf
			GUICtrlSetData($idLbl_PreBetaVer, 'Version: ' & $g_sPreBetaVer)
			GUICtrlSetData($idLbl_ReleaseDate, 'Date: ' & _FriendlyDate($g_iReleaseDate))
			GUICtrlSetData($idLbl_BetaDate, 'Date: ' & _FriendlyDate($g_iBetaDate))
			GUICtrlSetData($idLbl_PreBetaDate, 'Date: ' & _FriendlyDate($g_iPreBetaDate))
	...
	ElseIf $a_Msg[1] = $hGui_About Then
		Select
			Case $a_Msg[0] = $idLbl_VisitSite_About
				_Start('https://www.autoitscript.com')
				;         Case $a_Msg[0] = $idLbl_About_ContactAuthor
...
Func _CancelDownload($i_Flag = 0)
	If $g_iDnInitiated Then
		...
		If $i_Response = 6 Then
			...
			EndIf
			GUICtrlSetData($g_idProgress_Ctrl, 0)
