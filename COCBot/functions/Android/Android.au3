...
Func WinGetAndroidHandle($bInitAndroid = Default, $bTestPid = False)
	...
	If $g_hAndroidWindow = 0 Then _WinGetAndroidHandle(True) ; Fallback: search by window title if WMI/process search failed
	If $g_hAndroidWindow = 0 Then
		; Final fallback: emulator-specific class-based window search (for BS5.22+ where title may differ)
		Local $sFindFunc = "Find" & $g_sAndroidEmulator & "WindowByClass"
		Local $hClassWin = Execute($sFindFunc & "()")
		If IsHWnd($hClassWin) And $hClassWin <> 0 Then
			SetDebugLog("Found " & $g_sAndroidEmulator & " window by class search: " & $hClassWin)
			UpdateHWnD($hClassWin)
			$g_sAndroidTitle = WinGetTitle($hClassWin)
		EndIf
	EndIf
